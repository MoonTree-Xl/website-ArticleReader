...
def info_organize(text:str) -> tuple:
    result_dict = {}
    # 创建聊天模型
...
def summary_model(api_key,file,memory,question,attachment=None,summary_button=None,store_button=None):
    # 创建临时文档
    file_content = file.read() # 读取用户上传的文件，返回二进制数据
    temp_file_path = 'temp.pdf' # 临时文件路径
    with open(temp_file_path,'wb') as temp_file:
        temp_file.write(file_content) # 向临时文件写入内容
    temp_attachment_path = 'website_Articleanalysis\\temp_attachment.CSV'
    ...
    if summary_button:
        ...
        def str_maker(text_list):
            ...
            return text_final
        # 执行AI函数
        summary_text = article_reader(temp_file_path)
        article_title,article_dict = info_organize(summary_text)
        # 生成最终结果
        article = article_dict[article_title]
